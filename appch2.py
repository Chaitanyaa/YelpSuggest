...
def suggest():
    ...
    if request.method == 'POST':
        ...
        if userid==None:
             redirect('/newuser',code=307)# Handle New user case------
...
def usersuggest():
    #userid = '_VMGbmIeK71rQGwOBWt_Kg'
    userid=request.form['userid'] ## New changes
    if request.method == 'POST':
...
def getmapColab():
    if session.get('user_id') != None:
        ...
        result_df = getCollabRecom(session.get('user_id'))##New Changes
