...
def suggest():
    ...
    if request.method == 'POST':
        ...
        if userid==None: # Handle New user case------
			return redirect('/newuser',code=307)
        ...
        return render_template('suggest.html',varieties=varieties,ambience=ambience,management=management,service=service,value=value,rating=finalres[finalres.index == id]['rating'][0],name=finalres[finalres.index == id]['business_name'][0])
...
def getCardsValue(u_id):
    ...
    liked_business = pq.ParquetDataset('s3://yelpsuggest/Data/Final_Result/liked_business.parquet',
...
def usersuggest():
    ...
    if request.form.get('search') == None:
        userid = request.form['userid']
        t_df = content_df[content_df['user_id'] == userid]
        if (len(userid) != 22):
            return redirect(url_for('newuser'))
        elif t_df['user_id'].count() > 0:
            if request.form.get('search') != None:
...
def getmap():
    if session.get('keyword') != None:
        ...
        result_df = contentRecommed('U4INQZOPSUaj8hMjLlZ3KA')
...
def getmapColab():
    result_df = getCollabRecom('_VMGbmIeK71rQGwOBWt_Kg')
    mp = folium.Map(location=[36.1699, -115.1398], zoom_start=12)
    for i, r in result_df.iterrows():
        folium.Marker(location=[float(r.latitude), float(r.longitude)],
                      popup=html.escape(r["business_name"]) + '<br>' + 'Stars: ' + str(
